...
class PlanApprovalTests(unittest.TestCase):
    ...
    def test_canonical_plan_rejects_unapproved_constructor_and_overrides(self) -> None:
        ...
        with self.assertRaisesRegex(PlanGraphError, "requires an approval receipt"):
            PlanGraph(plan, lambda request: None, run_root=self.root / "runs-a")
        with self.assertRaisesRegex(PlanGraphError, "forbids functionality-test overrides"):
            PlanGraph(
                plan,
                lambda request: None,
                run_root=self.root / "runs-b",
                functionality_tests=[("true",)],
                approval_validator=lambda: None,
            )
