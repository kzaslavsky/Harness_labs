...
- If a repair regresses a closed closure, keep the closed closure frozen and reject the individual repair. Give a fresh fixer the controller-written collision packet for both closures and require both immutable tests to pass together. Permit two collision attempts, expand the packet once to at most three closures, and then require operator resolution; never resume isolated repair of a collision member.
