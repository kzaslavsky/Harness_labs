...
The shipped PlanGraph CLI now admits only a committed canonical
`plan-graph-plan/1` decomposition with an operator-attested approval receipt.
Use `scripts/approve_plan.py prepare` to freeze and gate the subject, record an
...
`scripts/run_plan_graph.py --approval-receipt`. The receipt binds the exact Git
