#!/usr/bin/env python3
"""Contract-burden relaxation 2 PlanGraph: the harness fixing its own contracts.

Eight nodes (CB2-01 … CB2-08), one per still-open finding cluster in
docs/development/contract-burden-reduction.md (items 8, 10, 11, 14, 15, 16).
Every red/green node's controller-owned gate is scripts/dev/red_green_check.py:
its finding tests must FAIL against the frozen post-adoption harness
(RED_BASE) and PASS on the candidate.

Stages:
  decompose — assemble docs/development/contract-burden-2-decomposition.json
...
PLAN_PATH = "docs/development/CONTRACT_BURDEN_RELAXATION_2_PLAN.md"
DECOMPOSITION_PATH = "docs/development/contract-burden-2-decomposition.json"
DIAGNOSIS_PATH = "docs/development/contract-burden-reduction.md"

LOGICAL_GRAPH_ID = "contract-burden-relaxation-2"

# The frozen post-adoption harness (CB-1 candidate merged) every finding test
# must fail against.
RED_BASE = "e605fffc90d880fc7e5bb3d779b82b29f74f8e20"
...
NODES: tuple[dict[str, Any], ...] = (
    ...
    {
        "id": "CB2-02",
        "finding_tests": ["tests/test_relax_gate_timeout_classification.py"],
        "regression": ["tests/"],
        "allowed_paths": [
...
def parse_plan_document() -> tuple[dict[str, str], dict[str, str], dict[str, str], dict[str, list[str]]]:
    ...
    for node_id, body in sections.items():
        ...
        ids: list[str] = []
        for ac in re.finditer(r"^- (AC-CB2\d\d-\d+): (.+)$", body, re.M):
...
def assemble_decomposition() -> dict[str, Any]:
    ...
    decomposition = {
        ...
        "referenced_artifacts": [
            ...
            "scripts/dev/red_green_check.py",
            "scripts/dev/check_workaround_retirement.py",
            "docs/development/CONTRACT_BURDEN_RELAXATION_PLAN.md",
        ...
        json.dumps(
            {
                ...
                "statement": (
                    "Operator-directed contract-burden relaxation 2 program. "
                    "Every node is bound to items verified GENUINELY OPEN in "
                    "the committed diagnosis document and carries a red/green "
                    "gate proving the finding against the frozen "
                    "post-adoption base harness."
                ),
            },
            indent=2,
            sort_keys=True,
...
def _launch_node(
    ...
    result = run_plan_graph_feature_worktree(
        ...
        worktree_path=WORKTREE_ROOT / f"cb2-{request.feature_run_id}",
...
def run_graph(run_id: str, receipt_path: Path) -> int:
    ...
    graph = PlanGraph(
        ...
        run_root=ROOT / "logs" / "runs" / "cb2-graph",
        graph_run_id=f"cb2-graph-{run_id}",
        logical_graph_id=LOGICAL_GRAPH_ID,
        approval_validator=admission.approval_validator(),
        # Ready-set execution: the CB2 DAG has three file-disjoint roots
        # (CB2-01 ∥ CB2-02 ∥ CB2-04) but program rule 2 admits at most two
        # nodes concurrently until CB2-03 seals load-aware budgets. Each node
        # still owns its own worktree and run_dir; joins are controller-owned
        # merge commits.
        max_parallelism=2,
    )
    result = graph.run()
...
def resume_graph(
    ...
    # The logical id is the stable graph slot across attempts. Unlike the CB-1
    # program, every CB2 root attempt passes logical_graph_id explicitly, so
    # the default --logical-id matches the fresh-run registration.
...
def main() -> int:
    ...
    run_id = arguments.run_id or "cb2-exp-1"
