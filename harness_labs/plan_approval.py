...
from .plan_graph import ApprovalEvidence, PlanGraph, plan_from_mapping
...
def _run_static_gates(
    *,
    ...
    graph = PlanGraph(
        plan,
        lambda request: (_ for _ in ()).throw(AssertionError("gate cannot launch")),
        run_root=repository / "logs" / "runs",
        approval_validator=lambda: ApprovalEvidence(
            subject_sha256, "0" * 64, identity
        ),
    )
    graph.validate()
    ...
    for index, command in enumerate(graph.functionality_tests):
