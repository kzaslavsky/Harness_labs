    ...
    def import_child_evidence(self, *, node_id: str, evidence: Mapping[str, object]) -> tuple[str, ...]:
        ...
        for repair in repairs:
            ...
            fact: dict[str, object] = {
                "invocation_id": invocation_id, "kind": "repair_dispatch", "classification": classification,
            }
            failure_keys = repair.get("failure_keys", ())
            if failure_keys:
                # Surfaces which tests motivated this repair dispatch so the
                # parent ledger's evidence trail carries the same substrate
                # `reserve(failure_keys=...)` uses, without itself spending
                # any per-finding budget: import is a retroactive record, not
                # a reservation.
                fact["failure_keys"] = list(self._failure_keys(failure_keys, None))
            facts.append(fact)
